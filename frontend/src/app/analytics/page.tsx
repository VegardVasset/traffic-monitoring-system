...
export default function AnalyticsPage() {
  ...
  const restFetchEvents = events.filter(
    (e) => e.message === "REST fetch completed" && e.data && typeof e.data.fetchDuration === "number"
  );
  const averageFetchDuration = useMemo(() => {
    if (restFetchEvents.length === 0) return 0;
    const total = restFetchEvents.reduce((acc, curr) => acc + curr.data.fetchDuration, 0);
    return total / restFetchEvents.length;
  }, [restFetchEvents]);

  // Aggregate metrics for live mode latency events
  const liveLatencyEvents = events.filter(
    (e) => e.message === "Live mode latency" && e.data && typeof e.data.latency === "number"
  );
  const averageLiveLatency = useMemo(() => {
    if (liveLatencyEvents.length === 0) return 0;
    const total = liveLatencyEvents.reduce((acc, curr) => acc + curr.data.latency, 0);
  ...
  const frequencyEvents = events.filter(
    (e) => e.message === "Data arrival frequency" && e.data && typeof e.data.count === "number"
  );
  const averageDataFrequency = useMemo(() => {
    if (frequencyEvents.length === 0) return 0;
    const total = frequencyEvents.reduce((acc, curr) => acc + curr.data.count, 0);
    ...
    <div className="p-4">
      ...
      <section className="mb-6">
        ...
        <ul className="list-disc ml-6">
          <li>Average REST Fetch Duration: {averageFetchDuration.toFixed(2)} ms</li>
          <li>Average Live Mode Latency: {averageLiveLatency.toFixed(2)} ms</li>
          <li>Average Data Arrival Frequency (per minute): {averageDataFrequency.toFixed(2)}</li>
        </ul>
      </section>
        ...
        <table className="w-full border-collapse">
          ...
          <tbody>
            {events.map((event, index) => (
              <tr key={index}>
                <td className="border p-2">{new Date(event.timestamp).toLocaleTimeString()}</td>
